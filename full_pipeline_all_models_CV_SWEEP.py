...
def cv_sweep(spec, X, y, subjects, out_dir):
    ...
    # Phase 2 (sequential): full CV sweep, load phase 1 checkpoint as starting weights.
    ...
    if is_phase2:
        if phase1_ckpt and os.path.exists(phase1_ckpt):
            print(f"  Phase 2 mode: loading phase 1 checkpoint: {phase1_ckpt}")
        else:
            print(f"  Phase 2 mode: WARNING checkpoint not found at {phase1_ckpt} — starting from pretrained weights")
